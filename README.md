...
这是一个面向现场演示的教学运行时：输入无图初中数学题目、简短参考答案和可选的多段参考解析，系统以这三项参考材料依据组织教学路线，经过整篇审稿和修订后生成语音、板书和选择式互动，最后进入横屏全屏课堂。这个 Demo 用于展示参考材料依据的 AI 讲解能达到什么质量，不是自动批改系统，也不承担通用自动解题。课程与生成任务只保存在当前服务进程的内存中，重启服务后会丢失。
...
- 工具报告 `unsupported` 表示“本地检查器不覆盖”，不会单独阻断讲解；只有安全/格式输入无效，或发现与参考结论直接相关且可复现的数学矛盾时才阻断；
...
→ 审查结果为 consistent 或 warning 时冻结 teaching route；只有结论相关的可复现矛盾阻断
...
- **结构化模型审阅与局部检查**约束提取出的方法、假设、推理步骤、结论和检查请求。它能保留 warning、阻断已重现且与结论相关的矛盾，不能证明模型发现了自然语言参考材料中的所有错误。
