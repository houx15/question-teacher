...
- 精简、标准两种讲解长度；
- 新课程中可自动评分的数学互动：`point_select`、`choice`，以及选择式近迁移互动；
...
新生成的可自动评分数学互动由确定性门禁限制为 `choice` 或 `point_select`。
表达式输入和旧式 `transfer` 互动仅为读取已有课程的兼容类型，生成链路不会再
产出它们。Director 与 Reviewer 的生成策略会把 `free_text` 留给低风险反思；
运行时仍支持它，且 API 始终返回 `needs_review`，不会自动评分。当前确定性门禁
不能从 prompt 的语义判断它是否真是反思，因此该约束仍是模型与人工审阅边界，
不能被表述成确定性的语义分类。
...
5. 新生成课程中的选择与点选由服务端判定。每个选择项会显示并朗读其专属诊断反馈；错误后继续按钮保持禁用，直到完成当前互动。
6. 表达式和旧式 `transfer` 输入仅作已有课程的兼容读取。运行时支持 `free_text`，API 固定返回 `needs_review`（“待确认”），不会自动评分。Director 与 Reviewer 的策略将其保留给低风险反思，但确定性门禁不能验证 prompt 的语义是否真是反思；这仍需模型与人工审阅。新生成的自动评分数学作答固定使用 `choice` 或 `point_select`。
