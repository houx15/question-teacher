...
- 精简、标准两种讲解长度；
- 新课程中可自动评分的数学互动：点选、选择，以及选择式近迁移互动；
- 仅用于低风险反思的 `free_text` 互动（固定为 `needs_review`，不自动判定）；
...
新生成的可自动评分数学互动只会使用选择或点选。表达式输入和旧式
`transfer` 互动仅为读取已有课程的兼容类型，生成链路不会再产出它们；
`free_text` 只允许用于低风险反思，固定标为 `needs_review`，不会自动判定。
...
6. 表达式和旧式 `transfer` 输入仅作已有课程的兼容读取。`free_text` 只允许用于低风险反思，固定标记为 `needs_review`（“待确认”），不会自动判错，也不会作为新生成的数学作答互动。
