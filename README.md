...
脚本会沿目录描述符逐级打开路径，拒绝跟随符号链接，并在创建和写入前后校验身份。
输出根和每一级受控目录必须属于当前有效用户，且不允许组或其他用户写入；
新目录固定为 `0700`，输出文件固定为 `0600`。
成功运行必须同时通过硬门槛审稿和 Schema/Runtime 验证；覆盖率按计数精确计算，
总数为零时按完整覆盖 `1.0` 记录。

安全边界是本地单用户评估：脚本防止符号链接穿越和其他用户或组对输出目录的写入，
不承诺隔离与评估进程拥有同一有效用户 ID 的主动恶意进程。如果需要运行不可信的本地代码，
应将评估放在专用系统账户或容器中。
